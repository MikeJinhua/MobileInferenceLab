"""Export fixed-shape SpatialSR2x to portable ExecuTorch and verify parity."""
...
def export_and_verify(output: Path) -> Dict[str, object]:
    ...
    # No partitioner: this task validates ExecuTorch portable kernels only.
    program = to_edge_transform_and_lower(exported_program).to_executorch()
    ...
    report = {
        ...
        "operators": operator_inventory(exported_program),
    ...
    if report["runtime_max_abs_difference"] > 1e-5:
        raise AssertionError(f"portable runtime parity failed: {report['runtime_max_abs_difference']}")
    return report


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--output", type=Path, default=Path("results/p2_3/spatial_sr_portable.pte"))
    parser.add_argument("--report", type=Path, default=Path("results/p2_3/portable_report.json"))
