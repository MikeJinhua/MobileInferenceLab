...
- **Phase 3:** start with the official Maven Central `executorch-android:1.3.1` AAR and its Java/Kotlin API; the AAR already wraps the native runtime and XNNPACK. First audit/pin the Android toolchain, then scaffold a minimal app, package a reproducibly generated static XNNPACK asset, implement RGB tensor conversion/inference/display, and finally measure load/preprocess/inference/postprocess/end-to-end timing on a connected device. Defer direct C++ integration until lower-level instrumentation or Vulkan interop requires it.
