...
> Status: Phase 1 — deterministic image inference plus local export-readiness preflight. ExecuTorch, Android, QNN, Vulkan, and temporal reconstruction are planned but not implemented.
...
See [SPEC](docs/SPEC.md), [PLAN](docs/PLAN.md), [TASKS](docs/TASKS.md), the [export-readiness report](docs/EXPORT_READINESS.md), and the [environment report](docs/ENVIRONMENT.md). Architecture diagrams, demo media, deployment findings, profiling, optimization history, and CPU/GPU/NPU comparisons will be added when verified evidence exists.
